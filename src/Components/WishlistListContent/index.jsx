...
const WishlistListContent = () => {
  const { finalList, listWishlist } = useContext(MyContext)
  const { currentUser } = useContext(AuthContext)


  const array = finalList.filter(item => {
    return listWishlist.some(obj => item.id === obj.id);
  });
  ...
  return (
    <section className="show-games__container">
      ...
      <ul className="show-games__list">
        {array.length !== 0 ? showContent() : <p className='empty-msg'>Você ainda não adicionou nada a sua lista de desejos.</p>}
      </ul>
    </section>
  )
}
